...
class Map:    
    ...
    def __init__(self):
        ...
        starting_zone = self.town_names.pop(self.town_names.index("Your House"))
    ...
    def placeZone(self,cur,dr,root = None):
        ...
        if root == None:
            ...
            home_flag = 0
        if len(self.wild_names) > 0 or len(self.town_names) > 0 or len(cur.neighbors) != 0:
            neighbors_to_place = random.randint(max(0,home_flag),min(3+home_flag, len(self.wild_names)))
            neighbor_list = []
            if cur.z_type != "town" and neighbors_to_place > 0 and len(self.town_names) > 0:
            ...
            while neighbors_to_place > 0:
                created_wild = zone.Wild(self.wild_names.pop(),dr,self.enemy_types,cur)
    ...
    def printMap(self):
        ...
        for i in range(len(self.current_zone.neighbors)):
            ...
            if len(self.current_zone.neighbors[i].neighbors) > 1:
                map_layer.append([])
                j_map += 1
                self.generateMapLayer(self.current_zone.neighbors[i], self.current_zone, map_layer[j_map])
            j_map += 1
        ...
        for i in range(len(cur_zone.neighbors) ):
            if cur_zone.neighbors[i].z_name != root_zone.z_name:
                ...
                if len(cur_zone.neighbors[i].neighbors) > 1:
                    map_layer.append([])
                    j_map += 1
                    self.generateMapLayer(cur_zone.neighbors[i], cur_zone, map_layer[j_map])
                j_map += 1
